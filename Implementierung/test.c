...
test_vector *setup_RFC2040_testvector(int padding, char *key, size_t key_length, char *
plain, int plain_length, uint32_t iv) {
...
static int cmp_to_RFC2040(char *plain, char *key, uint32_t iv, uint16_t *savedRoundkeys) {
    ...
    memcpy(dataRfc, dataOur, size_all);

    // Verschlüssele Plaintext + padding mit der RFC2040-Implementierung
    test_vector *ptv = setup_RFC2040_testvector(1, key, strlen(key), dataRfc, size_all, iv);
